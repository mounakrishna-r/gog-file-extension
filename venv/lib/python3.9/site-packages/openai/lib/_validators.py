...
def common_prompt_prefix_validator(df: pd.DataFrame) -> Remediation:
    ...
    if common_prefix != "":
        ...
        if MAX_PREFIX_LEN < len(common_prefix):
            immediate_msg += ". Fine-tuning doesn't require the instruction specifying the task, or a few-shot examples scenario. Most of the time you should only add the input data into the prompt, and the desired output into the completion"
