...
would be applying regular expression examples on the ``PATH_INFO`` and
...
Here a simple examples that creates a URL map for an application with
...
endpoint in the rules can be anything, for examples import paths or
...
method (for examples there were only rules for ``GET`` and ``HEAD`` but
