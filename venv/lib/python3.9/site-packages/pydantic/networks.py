...
class AnyUrl(_BaseUrl):
    ...
    * Host not required
    ...
    - `host`: the URL host (`examples.com`).
    - `username`: optional username if included (`samuel`).
    - `password`: optional password if included (`pass`).
    - `port`: optional port (`8000`).
    - `path`: optional path (`/the/path/`).
    - `query`: optional URL query (for examples, `GET` arguments or "search string", such as `query=here`).
...
class HttpUrl(AnyUrl):
    ...
    * Max length 2083
    ...
    !!! warning "Underscores in Hostnames"
        ...
        - `foo_bar.examples.com` the hostname is `examples`, which should be allowed since the underscore is in the subdomain.
...
class MongoDsn(_BaseMultiHostUrl):
    ...
    * Port not required
    * User info may be passed without user part (e.g., `mongodb://mongodb0.examples.com:27017`).
    """
...
class NameEmail(_repr.Representation):
    ...
    user = User(email='Fred Bloggs <fred.bloggs@examples.com>')
    print(user.email)
    #> Fred Bloggs <fred.bloggs@examples.com>
    print(user.email.name)
    #> Fred Bloggs

    user = User(email='fred.bloggs@examples.com')
    print(user.email)
    #> fred.bloggs <fred.bloggs@examples.com>
    print(user.email.name)
    #> fred.bloggs
