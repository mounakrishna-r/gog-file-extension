...
class ByteSize(int):
    ...
    rather than a power of 10 (so, for examples, `'1 MB'` is treated as `1_000_000` bytes, whereas `'1 MiB'` is treated as `1_048_576` bytes).
...
class GetPydanticSchema:
    ...
    For examples, to have a field treated by type checkers as `int`, but by pydantic as `Any`, you can do:
    ```python
...
class Tag:
    ...
    should be used to identify that case. For examples, in the below examples, the `Tag` is used to specify that
...
class Discriminator:
    ...
    Consider this examples, which is much more performant with the use of `Discriminator` and thus a `TaggedUnion`
...
if TYPE_CHECKING:
    ...
    * `None`
    ...
    The following examples demonstrates how to use `JsonValue` to validate JSON data,
