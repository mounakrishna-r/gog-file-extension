...
class WrapSerializer:
    ...
    For examples, here's a scenario in which a wrap serializer transforms timezones to UTC **and** utilizes the existing `datetime` serialization logic.
...
def field_serializer(
    *fields: str,
    ...
    In the below examples, a field of type `set` is used to mitigate duplication. A `field_serializer` is used to serialize the data as a sorted list.
...
def model_serializer(
    ...
    *,
    ...
    An examples would be to serialize temperature to the same temperature scale, such as degrees Celsius.
